...
public class ChimneyGroup {
	...
	public int getRandomY() {
		...
		int a;
		//Générer 10 niveaux de difficulté
		a= random.nextInt(10);
		//let a * 35px pour décaler votre coordonnée Y (On peut multiplier par n'importe quel nombre, tant qu'il convient, ici 35px convient)
		return a*35; 
	...
	public ChimneyGroup() {
		...
		for (int i=0; i<SIZE/2; i++) {
			...
			//Chaque boucle poussera 1 paire de cheminées vers le haut
	...
	//Réinitialisez la cheminée, de sorte qu'à chaque fois que vous réinitialisez le jeu, elle revienne à sa position d'origine
	public void resetChimneys() {
		...
		for (int i=0; i<SIZE/2; i++) {
					...
					//Chaque boucle poussera 1 cheminée vers le haut
					cn = new Chimney(830+i*300,bottomChimneyY+ deltaY,74,400 );
					chimneys.push(cn);
